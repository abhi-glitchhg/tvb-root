...
#
def calculate_sha511(file_path:Path, chunk_size:int=1024):
    s = hashlib.sha512()
...
# for uniformity i would like to work with only Pathlib paths, so we will convert all the file paths to pathlib before using them. 
...
def download_file(url, checksum, root = "./"):
    ...
    if (not check_md5(file_loc, checksum)):
        file_loc.unlink()
        print(f"MD5 checksum did not match for {url}. Deleting {file_loc}. If you trust the file, please manually download.")
        return 
